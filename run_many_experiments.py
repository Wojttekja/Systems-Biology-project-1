...
def _resolve_targets(targets: list[str], prefixes: list[str]) -> list[Path]:
    ...
    for prefix in prefixes:
        ...
        if not matched:
            print(
                f"  [warn] --prefix '{prefix}' matched no files in {EXPERIMENTS_DIR}/"
            )
    ...
    for target in targets:
        ...
        if p.is_dir():
            ...
            if by_prefix:
                print(
                    f"  [info] '{target}' not found as path; "
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        metavar="PREFIX",
        help=(
            "Match all experiments/<PREFIX>*.json files. Can be used multiple times."
        ),
    )
    parser.add_argument(
        "--workers",
    ...
    if not args.yes:
        try:
            answer = (
                input(f"Run all {len(configs)} experiment(s)? [y/N] ").strip().lower()
            )
